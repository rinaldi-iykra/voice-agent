...
async def root_redirect():
    ...
    from pathlib import Path
    summary_file = Path(__file__).parent / "static" / "index.html"
...
async def offer(request: dict, background_tasks: BackgroundTasks):
    ...
    voice_provider = request.get("voice_provider", "azure")  # Get voice provider from request, default to "azure"
    ...
    if pc_id and pc_id in pcs_map:
        ...
        # Run bot function with proper mode string and voice provider
...
async def get_tts_status():
    """Get current TTS latency and voice provider information"""
    try:
        # Get the most recent active connection
        if pcs_map:
            latest_connection = list(pcs_map.values())[-1]
            latency = getattr(latest_connection, 'last_tts_latency', None)
