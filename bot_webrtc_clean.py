...
from pipecat.frames.frames import TranscriptionMessage, TranscriptionUpdateFrame, UserStoppedSpeakingFrame, BotStartedSpeakingFrame
...
async def run_bot(webrtc_connection: SmallWebRTCConnection, mode="inbound", voice_provider="azure"):
...
Please transcribe BSI Syariah exactly as spoken, not as 'Bank Sentral'.""",
    ...
    # Create TTS service based on voice_provider parameter
    tts_start_time = None
    
    # Log TTS service selection with prominent banner
    logger.info("="*80)
    logger.info(f"🎤 INITIALIZING TTS SERVICE: {voice_provider.upper() if voice_provider else 'AZURE (DEFAULT)'}")
    logger.info("="*80)
    
    if voice_provider == "elevenlabs":
        tts = ElevenLabsTTSService(
            api_key=os.getenv("ELEVEN_API_KEY"),
            voice_id=os.getenv("ELEVEN_VOICE_ID", "n2asWyFQpoCQZU0YDIGK"),
        ...
        logger.info(f"🔥 ELEVENLABS TTS SERVICE ACTIVE 🔥")
        logger.info(f"✅ ElevenLabs API Key: {'✅ Set' if os.getenv('ELEVEN_API_KEY') else '❌ Not set'}")
        logger.info(f"✅ ElevenLabs Voice ID: {os.getenv('ELEVEN_VOICE_ID', 'n2asWyFQpoCQZU0YDIGK')}")
        logger.info(f"✅ ElevenLabs Model: eleven_multilingual_v2")
        ...
        logger.info(f"🤖 OPENAI TTS SERVICE ACTIVE 🤖")
        logger.info(f"✅ OpenAI API Key: {'✅ Set' if os.getenv('OPENAI_API_KEY') else '❌ Not set'}")
        logger.info(f"✅ OpenAI Voice: shimmer")
        logger.info(f"✅ OpenAI Model: gpt-4o-mini-tts")
    elif voice_provider == "murf":
        tts = MurfTTSService(
        ...
        logger.info(f"🎭 MURF TTS SERVICE ACTIVE 🎭")
        logger.info(f"✅ Murf API Key: {'✅ Set' if os.getenv('MURF_API_KEY') else '❌ Not set'}")
        logger.info(f"✅ Murf Voice ID: en-UK-ruby")
        logger.info(f"✅ Murf Style: Narration")
        logger.info(f"✅ Murf Locale: id-ID")
    else:
        tts = AzureTTSService(
        ...
        logger.info(f"☁️ AZURE TTS SERVICE ACTIVE (DEFAULT) ☁️")
        logger.info(f"✅ Azure Speech Key: {'✅ Set' if os.getenv('SPEECH_KEY') else '❌ Not set'}")
        logger.info(f"✅ Azure Voice: en-US-AvaMultilingualNeural")
        logger.info(f"✅ Azure Region: southeastasia")
        logger.info(f"✅ Azure Language: id-ID")
    
    # Add TTS debugging with enhanced visibility
    logger.info("─"*50)
    logger.info(f"🔧 TTS SERVICE DETAILS:")
    logger.info(f"   • Service Type: {type(tts).__name__}")
    logger.info(f"   • Service ID: {id(tts)}")
    logger.info(f"   • Voice Provider: {voice_provider or 'azure (default)'}")
    logger.info("─"*50)
    
    # Track user speech timing for latency analysis
    user_stop_time = None
    
    @tts.event_handler("on_tts_start")
    async def on_tts_start(tts, text):
        nonlocal tts_start_time, user_stop_time
        import time
        tts_start_time = time.time()
        
        # Calculate response latency if we have user stop time
        if user_stop_time:
            response_latency = (tts_start_time - user_stop_time) * 1000  # Convert to milliseconds
            logger.info(f"📊 Response Latency: {response_latency:.2f}ms (from user stop to TTS start)")
            # Store response latency for dashboard display
            if hasattr(webrtc_connection, 'last_response_latency'):
                webrtc_connection.last_response_latency = response_latency
            else:
                webrtc_connection.last_response_latency = response_latency
        
        logger.info(f"🔊 TTS Started ({voice_provider}): '{text}'")
    ...
    async def on_tts_end(tts, audio):
        ...
        import time
        if tts_start_time:
            latency = (time.time() - tts_start_time) * 1000  # Convert to milliseconds
            logger.info(f"🔊 TTS Ended ({voice_provider}): {len(audio) if audio else 0} bytes, Latency: {latency:.2f}ms")
            # Store latency for dashboard display
            if hasattr(webrtc_connection, 'last_tts_latency'):
                webrtc_connection.last_tts_latency = latency
                webrtc_connection.voice_provider = voice_provider
        else:
            logger.info(f"🔊 TTS Ended ({voice_provider}): {len(audio) if audio else 0} bytes")
    
    @tts.event_handler("on_tts_error")
    async def on_tts_error(tts, error):
        logger.error(f"🔊 TTS Error ({voice_provider}): {error}")
    ...
    detailed_prompt = (
        ...
        "6. Jangan memanggil nama nasabah. Panggil saja dengan nasabah yang terhormat.\n"
    ...
    # Add speech event handlers for latency tracking
    ...
    async def on_frame(transport, frame):
        ...
        if isinstance(frame, UserStoppedSpeakingFrame):
            ...
            logger.info(f"🎤 User stopped speaking at {user_stop_time}")
            # Store the timestamp for response latency calculation
            if hasattr(webrtc_connection, 'user_stop_time'):
                webrtc_connection.user_stop_time = user_stop_time
            else:
                webrtc_connection.user_stop_time = user_stop_time
...
async def offer(request: dict, background_tasks: BackgroundTasks):
    ...
    voice_provider = request.get("voice_provider", "azure")  # default to azure if not set
    ...
    if pc_id and pc_id in pcs_map:
        ...
        # Pass mode and voice_provider to run_bot
        background_tasks.add_task(run_bot, pipecat_connection, mode, voice_provider)
...
async def get_tts_status():
    """Get current TTS latency, response latency, and voice provider information"""
    try:
        # Get the most recent active connection
        if pcs_map:
            latest_connection = list(pcs_map.values())[-1]
            tts_latency = getattr(latest_connection, 'last_tts_latency', None)
            response_latency = getattr(latest_connection, 'last_response_latency', None)
            voice_provider = getattr(latest_connection, 'voice_provider', 'unknown')
        ...
        return {"error": "Failed to get TTS status"}
